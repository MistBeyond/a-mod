...
|--------------------|------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------|
| `block`            | Blocks, block entities, and block-level contracts (e.g. `WireBlock`, `IConnectable`)           | Blocks, block entities, connection and shape contracts owned by blocks    | Energy network logic, generic transfer math                               |
...
| `core/logistic`    | Logistic and energy domain logic and interfaces; keep this area pure                           | Interfaces, pure data, and logic; implementations in `impl/`              | Block- or item-specific behavior                                          |
| `core/registry`    | Registry infrastructure; contracts in `core/registry`, implementations in `core/registry/impl` | Registration contracts and implementations                                | Feature registration glue                                                 |
